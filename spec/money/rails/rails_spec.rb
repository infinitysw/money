...
class RoundingMoneyExample < ActiveRecord::Base
  set_table_name "money_examples"

  money :rounded_amount, :allow_nil => false, :cents => "debit_amount_in_cents", :precision => 5, :round => 2
end
...
describe Money, "using the money declaration in an ActiveRecord model" do
  describe "setter method" do
    ...
    it "should allow rounding when money is being set" do
      rme = RoundingMoneyExample.new(:rounded_amount => 1.23456)
      rme.rounded_amount.should == 1.23.to_money(5)

      rme.rounded_amount = 2.34567
      rme.rounded_amount.should == 2.35.to_money(5)
    end
  end
